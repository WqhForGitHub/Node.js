...
function renderNote(n) {
  console.log();
  console.log(
    color(`  #${n.id} ${n.pinned ? "★ " : ""}${n.title}`, C.bold + C.cyan),
  );
  console.log(
    color(
      `  创建: ${formatDate(n.createdAt)}    更新: ${formatDate(n.updatedAt)}`,
      C.gray,
    ),
  );
  if (n.tags.length > 0) {
...
async function cmdAdd(args) {
  ...
  try {
    ...
    const tagsInput = (
      await ask(rl, color("标签 (用空格或逗号分隔，可空): ", C.cyan))
    ).trim();
...
function cmdList(args) {
  ...
  console.log(
    color(
      `  共 ${notes.length} 条笔记` + (args.tag ? ` (标签 #${args.tag})` : ""),
      C.bold,
    ),
  );
  console.log();
...
async function cmdEdit(args) {
  ...
  try {
    const newTitle = (
      await ask(rl, color(`新标题 (回车保留 "${n.title}"): `, C.cyan))
    ).trim();
    const newTags = (
      await ask(
        rl,
        color(`新标签 (回车保留 "${n.tags.join(",")}"，输入 - 清空): `, C.cyan),
      )
    ).trim();
    ...
    if (args.editor) {
      ...
      const ans = (
        await ask(
          rl,
          color("修改内容? (e=外部编辑器 / r=重新输入 / 回车=保留): ", C.cyan),
        )
      )
        .trim()
        .toLowerCase();
...
async function cmdDelete(args) {
  ...
  if (!args.yes) {
    const rl = createRL();
    const ans = (
      await ask(rl, color(`确认删除 #${n.id} "${n.title}" ? (y/N) `, C.yellow))
    )
      .trim()
      .toLowerCase();
    rl.close();
...
function cmdSearch(args) {
  ...
  if (!keyword)
    return console.log(color("用法: search <关键字> [--tag <tag>]", C.yellow));
  const data = loadNotes();
...
function cmdPin(args, pin) {
  const id = args._[0];
  if (!id)
    return console.log(color(`用法: ${pin ? "pin" : "unpin"} <id>`, C.yellow));
  const data = loadNotes();
  const n = findNote(data, id);
  if (!n) return console.log(color(`未找到笔记 #${id}`, C.red));
  n.pinned = pin;
...
function cmdTags() {
  ...
  for (const [tag, count] of arr) {
    console.log(
      "  " +
        color(("#" + tag).padEnd(20), C.magenta) +
        color(String(count) + " 条", C.gray),
    );
  }
  console.log();
}
...
function cmdStats() {
  ...
  const totalChars = data.notes.reduce(
    (s, n) => s + (n.content ? n.content.length : 0),
    0,
  );
