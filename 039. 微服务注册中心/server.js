...
function addWatcher(res, lastEventId, timeout) {
  return new Promise((resolve) => {
    ...
    // 先检查是否有新事件
    const events = loadEvents();
    const newEvents = lastEventId ? events.filter((e) => e.id > lastEventId) : events.slice(-20);
...
function registerService(reqBody) {
  ...
  if (service) {
    // 检查是否已有相同地址的实例
    const existing = service.instances.find((i) => i.host === host && i.port === parseInt(port));
    if (existing) {
      // 更新心跳
...
function checkHeartbeats() {
    ...
    for (const instance of service.instances) {
      ...
      if (now - lastBeat > HEARTBEAT_TIMEOUT) {
        if (instance.healthy) {
          ...
          changed = true;
          console.log(`  [心跳超时] ${service.serviceName} ${instance.host}:${instance.port}`);
          addEvent('instance_unhealthy', service.serviceId, instance.instanceId, {
            host: instance.host,
            port: instance.port,
        ...
        // 可选: 超时后自动注销
        if (DEREGISTER_AFTER_TIMEOUT && now - lastBeat > HEARTBEAT_TIMEOUT * 3) {
          console.log(`  [自动注销] ${service.serviceName} ${instance.host}:${instance.port}`);
          addEvent('instance_auto_deregistered', service.serviceId, instance.instanceId, {
            host: instance.host,
            port: instance.port,
  ...
  for (let si = services.length - 1; si >= 0; si--) {
    ...
    for (let ii = service.instances.length - 1; ii >= 0; ii--) {
      ...
      const lastBeat = new Date(instance.lastHeartbeat).getTime();
      if (DEREGISTER_AFTER_TIMEOUT && now - lastBeat > HEARTBEAT_TIMEOUT * 3 && !instance.healthy) {
...
function listServices(req, res) {
  ...
  if (status) {
    services = services.filter((s) => s.instances.some((i) => i.status === status));
  }
...
async function updateServiceMetadata(req, res, serviceId) {
  ...
  if (body.metadata) service.metadata = { ...service.metadata, ...body.metadata };
...
async function updateInstanceStatus(req, res, serviceId, instanceId) {
  ...
  if (body.metadata) instance.metadata = { ...instance.metadata, ...body.metadata };
...
function getServiceDependencies(req, res, serviceId) {
  ...
  const depServices = service.dependencies.map((depId) => {
    const dep = services.find((s) => s.serviceId === depId || s.serviceName === depId);
    return dep
      ? {
...
function getServiceTopology(req, res) {
  ...
  for (const service of services) {
    for (const dep of service.dependencies) {
      const target = services.find((s) => s.serviceId === dep || s.serviceName === dep);
      if (target) {
        edges.push({
...
async function createCluster(req, res) {
  ...
  const clusters = loadClusters();
  if (clusters.find((c) => c.name === name)) return sendError(res, 409, '集群已存在');
...
async function handleRequest(req, res) {
  ...
  try {
    ...
    if (segments[0] === 'api' && segments[1] === 'services' && segments[2]) {
      ...
      if (!segments[3] && method === 'GET') return getService(req, res, serviceId);
    ...
    // 服务拓扑
    if (segments[0] === 'api' && segments[1] === 'topology' && method === 'GET') {
...
async function handleDeregisterViaBody(req, res) {
  ...
  if (!serviceId || !instanceId) return sendError(res, 400, '缺少 serviceId 或 instanceId');
