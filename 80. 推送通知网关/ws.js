...
class WSConnection {
  constructor(socket) {
    this.socket = socket; this.buffer = Buffer.alloc(0); this.handlers = {};
    socket.on('data', (chunk) => {
      ...
      while (true) {
        ...
        try { (this.handlers.message || []).forEach(h => h(JSON.parse(frame.payload.toString()))); } catch (_) {}
      }
    });
  ...
  send(data) { try { this.socket.write(encode(data)); } catch (_) {} }
