...
class PushService {
  ...
  register(userId: string, token: string, platform: string) {
    ...
    if (!['ios', 'android', 'web'].includes(platform))
      throw new Error('非法 platform: ' + platform);
  ...
  push(userId: string, title: string, body: string) {
    ...
    const record = this.repo.addRecord({
      id: Date.now() + Math.floor(Math.random() * 1000),
      userId,
      title,
      body,
...
router.delete('/api/devices/:token', (ctx) => {
  try {
    ctx.body = service.unregister(ctx.params.token);
  } catch (e) {
    ctx.status = 404;
    ctx.body = { message: (e as Error).message };
  }
...
router.post('/api/push', (ctx) => {
  try {
    ...
    ctx.status = m === '用户无可用设备' ? 404 : 400;
    ctx.body = { message: m };
  }
});
router.get('/api/push/records', (ctx) => {
