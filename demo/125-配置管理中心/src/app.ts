...
interface AppInfo { name: string; createdAt: number; }
interface PublishRecord { app: string; env: Env; count: number; publishedAt: number; }
// ---- Repository 层 ----
class ConfigRepository {
  ...
  findApp(name: string) { return this.apps.find((a) => a.name === name); }
  ...
  findById(id: number) { return this.configs.find((c) => c.id === id); }
...
class ConfigService {
  ...
  registerApp(name: string) { return this.repo.registerApp(name); }
  addConfig(app: string, body: any) {
    if (!body || !body.key || body.value === undefined || !body.env) throw new Error('参数缺失: key/value/env');
...
router.post('/api/apps', (ctx) => {
  try { ctx.status = 201; ctx.body = service.registerApp((ctx.request.body as any)?.name); }
  catch (e: any) { ctx.status = 400; ctx.body = { message: e.message }; }
});
// 添加配置
router.post('/api/apps/:app/configs', (ctx) => {
  try { ctx.status = 201; ctx.body = service.addConfig(ctx.params.app, ctx.request.body); }
  catch (e: any) { ctx.status = e.message === 'not found' ? 404 : 400; ctx.body = { message: e.message }; }
});
// 查询配置（按 env 过滤）
router.get('/api/apps/:app/configs', (ctx) => {
  try { ctx.body = service.listConfigs(ctx.params.app, ctx.query.env as Env | undefined); }
  catch (e: any) { ctx.status = e.message === 'not found' ? 404 : 400; ctx.body = { message: e.message }; }
});
// 更新配置
...
router.post('/api/apps/:app/publish', (ctx) => {
  try { ctx.body = service.publish(ctx.params.app, ctx.request.body); }
  catch (e: any) { ctx.status = e.message === 'not found' ? 404 : 400; ctx.body = { message: e.message }; }
});
