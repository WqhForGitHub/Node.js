...
class ReportRepository {
  ...
  addRows(rows: ReportRow[]) {
    this.rows.push(...rows);
  }
  findRows(reportId: number, dimension?: string) {
    return this.rows.filter(
      (r) => r.reportId === reportId && (!dimension || r.dimension === dimension),
    );
  }
}
// ---- Service 层 ----
class ReportService {
  ...
  define(name: string, dimensions: string[], metrics: string[]) {
    if (!name || !dimensions || !dimensions.length || !metrics || !metrics.length)
      throw new Error('参数缺失: name/dimensions/metrics');
...
router.post('/api/reports/:id/generate', (ctx) => {
  try {
    ctx.body = service.generate(Number(ctx.params.id));
  } catch (e) {
    const msg = (e as Error).message;
    ctx.status = msg === '报表不存在' ? 404 : 400;
    ctx.body = { message: msg };
  }
});
router.get('/api/reports/:id/data', (ctx) => {
  try {
    ctx.body = service.data(Number(ctx.params.id), ctx.query.dimension as string | undefined);
  } catch (e) {
    ctx.status = 404;
    ctx.body = { message: (e as Error).message };
  }
