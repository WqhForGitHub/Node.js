...
class NotificationRepository {
  ...
  unreadCount(userId: string) {
    return this.list.filter((n) => n.userId === userId && !n.read).length;
  }
}
// ---- Service 层 ----
class NotificationService {
  ...
  send(userId: string, title: string, content: string, type: string) {
    if (!userId || !title || !content || !type)
      throw new Error('参数缺失: userId/title/content/type');
...
router.post('/api/notifications/read-all', (ctx) => {
  try {
    ...
    ctx.body = service.markAllRead(userId);
  } catch (e) {
    ctx.status = 400;
    ctx.body = { message: (e as Error).message };
  }
...
router.get('/api/notifications/unread-count', (ctx) => {
  try {
    ...
    ctx.body = service.unreadCount(userId);
  } catch (e) {
    ctx.status = 400;
    ctx.body = { message: (e as Error).message };
  }
...
router.post('/api/notifications/:id/read', (ctx) => {
  try {
    ctx.body = service.markRead(Number(ctx.params.id));
  } catch (e) {
    const m = (e as Error).message;
    ctx.status = m === '通知不存在' ? 404 : 400;
    ctx.body = { message: m };
  }
});
