...
class UserRepository {
  private items: User[] = [{ id: 1, username: 'sample', email: 'sample', role: 'sample' } as User];
  findAll(): User[] {
    return this.items;
  }
  ...
  create(data: Partial<User>): User {
    const item = { id: Date.now(), ...data } as User;
  ...
  update(id: number, data: Partial<User>): User | undefined {
    ...
    this.items[idx] = { ...this.items[idx], ...data } as User;
...
app.get('/health', (_req: Request, res: Response) => {
  res.json({ status: 'ok', service: '用户管理系统' });
});
app.get('/api/users', controller.list.bind(controller));
app.get('/api/users/:id', controller.get.bind(controller));
