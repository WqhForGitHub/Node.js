 ...
 * 短信模板、发送、回执
 */
interface SmsTemplate { code: string; content: string; }
interface SmsRecord { id: number; templateCode: string; phone: string; content: string; status: 'queued' | 'delivered' | 'failed'; createdAt: number; deliveredAt: number | null; }

// ---- Repository 层 ----
class SmsRepository {
  private templates: Map<string, SmsTemplate> = new Map();
  private records: SmsRecord[] = [];
  addTemplate(t: SmsTemplate) { this.templates.set(t.code, t); return t; }
  findTemplate(code: string) { return this.templates.get(code); }
  listTemplates() { return Array.from(this.templates.values()); }
  addRecord(r: SmsRecord) { this.records.push(r); return r; }
  findRecord(id: number) { return this.records.find((r) => r.id === id); }
...
class SmsService {
  ...
  render(content: string, params: Record<string, string>) {
    return content.replace(/\$\{(\w+)\}/g, (_, k) => (params[k] !== undefined ? params[k] : '${' + k + '}'));
  }
  send(templateCode: string, phone: string, params: Record<string, string>) {
    ...
    return this.repo.addRecord({ id: Date.now() + Math.floor(Math.random() * 1000), templateCode, phone, content, status: 'queued', createdAt: Date.now(), deliveredAt: null });
  }
...
router.get('/api/sms/templates', (ctx) => { ctx.body = service.listTemplates(); });
...
router.post('/api/sms/send', (ctx) => {
  ...
  } catch (e) { const m = (e as Error).message; ctx.status = m === '模板不存在' ? 404 : 400; ctx.body = { message: m }; }
});
router.get('/api/sms/records', (ctx) => { ctx.body = service.listRecords(); });
router.post('/api/sms/:id/callback', (ctx) => {
  ...
  } catch (e) { const m = (e as Error).message; ctx.status = m === '短信记录不存在' ? 404 : 400; ctx.body = { message: m }; }
});
