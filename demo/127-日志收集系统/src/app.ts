...
interface Agent { id: string; source: string; lastHeartbeat: number; }
// ---- Repository 层 ----
class LogRepository {
  ...
  heartbeat(agentId: string, source: string) {
    ...
    if (!a) { a = { id: agentId, source, lastHeartbeat: Date.now() }; this.agents.push(a); }
    else { a.lastHeartbeat = Date.now(); a.source = source; }
    return a;
  }
  ...
  query(filter: { source?: string; level?: Level; from?: number; to?: number; page: number; size: number }) {
    ...
    return { total, page: filter.page, size: filter.size, data: list.slice(start, start + filter.size) };
  }
  sources() {
  ...
  stats() {
    ...
    this.logs.forEach((l) => { map[l.source] = (map[l.source] || 0) + 1; });
...
class LogService {
  ...
  ingest(body: any) {
    if (!body || !body.source || !Array.isArray(body.logs)) throw new Error('参数缺失: source/logs');
  ...
  sources() { return this.repo.sources(); }
...
router.post('/api/agents/heartbeat', (ctx) => {
  try { ctx.body = service.heartbeat(ctx.request.body); }
  catch (e: any) { ctx.status = 400; ctx.body = { message: e.message }; }
});
// 批量日志接入
router.post('/api/logs/ingest', (ctx) => {
  try { ctx.status = 201; ctx.body = service.ingest(ctx.request.body); }
  catch (e: any) { ctx.status = 400; ctx.body = { message: e.message }; }
});
// 多条件过滤 + 分页
