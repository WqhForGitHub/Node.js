...
class PermissionRepository {
  ...
  create(data: Partial<Permission>): Permission {
    const item = { id: Date.now(), ...data } as Permission;
    this.items.push(item);
    return item;
  }
  update(id: number, data: Partial<Permission>): Permission | undefined {
    const idx = this.items.findIndex((i) => i.id === id);
    if (idx < 0) return undefined;
    this.items[idx] = { ...this.items[idx], ...data } as Permission;
    return this.items[idx];
  }
  delete(id: number): Permission | undefined {
    const idx = this.items.findIndex((i) => i.id === id);
    if (idx < 0) return undefined;
...
class PermissionService {
  ...
  get(id: number): Permission | undefined {
  ...
  update(id: number, data: Partial<Permission>): Permission {
    const item = this.repo.update(id, data);
    if (!item) throw new Error('未找到资源');
    return item;
  }
  delete(id: number): Permission {
...
app.get('/health', (_req: Request, res: Response) => {
  res.json({ status: 'ok', service: '权限管理系统' });
});
app.get('/api/permissions', controller.list.bind(controller));
app.get('/api/permissions/:id', controller.get.bind(controller));
app.post('/api/permissions', controller.create.bind(controller));
app.put('/api/permissions/:id', controller.update.bind(controller));
app.delete('/api/permissions/:id', controller.delete.bind(controller));
...
app.listen(PORT, () => {
  console.log('[权限管理系统] running at http://localhost:' + PORT);
});
