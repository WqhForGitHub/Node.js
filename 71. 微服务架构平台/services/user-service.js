...
function hashPassword(password) {
  return crypto.createHash('sha256').update(password + 'salt-demo').digest('hex');
}
...
function createToken(payload) {
  ...
  const body = base64UrlEncode(JSON.stringify({ ...payload, exp: Date.now() + 86400000 }));
...
router.post('/api/users/login', async (req, res) => {
  ...
  const token = createToken({ userId: foundUser.id, username: foundUser.username, role: foundUser.role });
