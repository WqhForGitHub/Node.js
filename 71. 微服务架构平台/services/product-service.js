...
const demoProducts = [
  {
    ...
    stock: 50,
    category: "电子产品",
    description: "Apple M3 Pro 芯片笔记本电脑",
  },
  {
    name: "iPhone 15 Pro",
    price: 8999,
    stock: 200,
    category: "电子产品",
    description: "Apple A17 Pro 芯片智能手机",
  },
  {
    name: "AirPods Pro 2",
    price: 1899,
...
router.get("/api/products", async (req, res) => {
  ...
  // 获取所有分类
  const categories = [
    ...new Set(Array.from(products.values()).map((p) => p.category)),
  ];
...
router.put("/api/products/:id/stock", async (req, res, params) => {
  ...
  if (newStock < 0) {
    return errorResponse(
      res,
      `库存不足，当前: ${product.stock}，尝试扣减: ${Math.abs(quantity)}`,
      409,
    );
  }
...
router.get("/health", async (req, res) => {
  successResponse(res, {
    ...
    totalStock: Array.from(products.values()).reduce(
      (sum, p) => sum + p.stock,
      0,
    ),
  });
