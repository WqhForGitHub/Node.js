...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    if (pathname === '/workflows' && req.method === 'GET')
      return send(res, 200, Object.keys(workflows));
    ...
    if (pathname === '/executions' && req.method === 'GET')
      return send(res, 200, engine.listExecutions());
...
server.listen(PORT, () => {
  ...
  console.log(
    '示例: curl -X POST http://localhost:3091/run/order_process -d \'{"input":{"amount":100}}\' -H "Content-Type: application/json"'
  );
});
