...
async function handleSet(req, res, ns, key) {
  ...
  const ttlMs =
    typeof body.ttl === "number" && body.ttl > 0 ? body.ttl : undefined;
...
async function handleExpire(req, res, ns, key) {
  ...
  if (typeof body.ttl !== "number" || body.ttl <= 0)
    return err(res, 400, "ttl 必须是正数（毫秒）");
  const entry = getEntry(ns, key);
  if (!entry) return err(res, 404, `键不存在: ${key}`);
...
async function handleHashGet(req, res, ns, key) {
  ...
  if (q.field) {
    if (!(q.field in entry.value))
      return err(res, 404, `字段不存在: ${q.field}`);
...
async function handleStats(req, res) {
  ...
  ok(res, {
    ...
    snapshotSize: fs.existsSync(SNAPSHOT_FILE)
      ? fs.statSync(SNAPSHOT_FILE).size
      : 0,
  });
}
    ...
    if (segments[0] === "api" && segments[1] === "kv" && segments.length >= 3) {
      ...
      if (segments.length >= 4) {
        ...
        if (segments.length === 5) {
          ...
          if (sub === "exists" && method === "GET")
            return await handleExists(req, res, ns, key);

          if (sub === "ttl" && method === "GET")
            return await handleTtl(req, res, ns, key);

          if (sub === "expire") {
            if (method === "PUT") return await handleExpire(req, res, ns, key);
            if (method === "DELETE")
              return await handlePersist(req, res, ns, key);
          }

          if (sub === "incr" && method === "POST")
            return await handleIncr(req, res, ns, key, 1);
          if (sub === "decr" && method === "POST")
            return await handleIncr(req, res, ns, key, -1);

          if (sub === "list" && method === "GET")
            return await handleListRange(req, res, ns, key);

          if (sub === "hash") {
            if (method === "GET") return await handleHashGet(req, res, ns, key);
            if (method === "PUT") return await handleHashSet(req, res, ns, key);
            if (method === "DELETE")
              return await handleHashDel(req, res, ns, key);
          }

          if (sub === "set" && method === "GET")
            return await handleSetMembers(req, res, ns, key);
        }
...
server.listen(PORT, () => {
  ...
  console.log(
    "     GET    /api/kv/:ns?pattern=*&limit=100                  - 列出命名空间下的键",
  );
  console.log(
    "     DELETE /api/kv/:ns                                     - 清空命名空间",
  );
  console.log(
    "     GET    /api/kv/:ns/:key                                - 读取键",
  );
  console.log(
    "     PUT    /api/kv/:ns/:key            body:{value,ttl?}   - 写入键",
  );
  console.log(
    "     DELETE /api/kv/:ns/:key                                - 删除键",
  );
  console.log(
    "     GET    /api/kv/:ns/:key/exists                         - 判断是否存在",
  );
  console.log(
    "     GET    /api/kv/:ns/:key/ttl                            - 查询 TTL",
  );
  console.log(
    "     PUT    /api/kv/:ns/:key/expire     body:{ttl}          - 设置过期时间(ms)",
  );
  console.log(
    "     DELETE /api/kv/:ns/:key/expire                         - 移除过期时间",
  );
  console.log(
    "     POST   /api/kv/:ns/:key/incr       body:{by?}          - 数值自增",
  );
  console.log(
    "     POST   /api/kv/:ns/:key/decr       body:{by?}          - 数值自减",
  );
  console.log(
    "     POST   /api/kv/:ns/:key/list/push  body:{values,direction?} - 列表追加",
  );
  console.log(
    "     POST   /api/kv/:ns/:key/list/pop   body:{direction?}   - 列表弹出",
  );
  console.log(
    "     GET    /api/kv/:ns/:key/list?start=0&end=-1            - 列表区间",
  );
  console.log(
    "     PUT    /api/kv/:ns/:key/hash       body:{fields}       - 哈希字段写入",
  );
  console.log(
    "     GET    /api/kv/:ns/:key/hash[?field=]                  - 哈希字段读取",
  );
  console.log(
    "     DELETE /api/kv/:ns/:key/hash       body:{fields}       - 哈希字段删除",
  );
  console.log(
    "     POST   /api/kv/:ns/:key/set/add    body:{members}      - 集合添加",
  );
  console.log(
    "     POST   /api/kv/:ns/:key/set/remove body:{members}      - 集合移除",
  );
  console.log(
    "     GET    /api/kv/:ns/:key/set                            - 集合成员",
  );
  console.log(
    "     POST   /api/admin/snapshot                             - 手动触发快照",
  );
  console.log(
    "     GET    /api/admin/stats                                - 服务状态",
  );
});
