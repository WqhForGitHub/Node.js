...
function log(method, path, statusCode, duration, target) {
  ...
  console.log(`[Gateway] ${timestamp} ${method} ${path} ${statusCode} ${duration}ms${targetStr}`);
}
...
function createToken(payload) {
  ...
  const signature = base64UrlEncode(
    require('crypto').createHmac('sha256', JWT_SECRET).update(`${header}.${body}`).digest('base64')
  );
...
// 不需要认证的路径
const PUBLIC_PATHS = ['/api/users/login', '/api/users/register', '/api/products', '/health'];
