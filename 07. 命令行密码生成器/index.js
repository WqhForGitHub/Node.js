...
function runInteractive() {
  ...
  (async () => {
    ...
    const lower =
      (await question(color("包含小写字母? (Y/n): ", "cyan"))).toLowerCase() !==
      "n";
    const upper =
      (await question(color("包含大写字母? (Y/n): ", "cyan"))).toLowerCase() !==
      "n";
    const number =
      (await question(color("包含数字? (Y/n): ", "cyan"))).toLowerCase() !==
      "n";
    const symbol =
      (await question(color("包含特殊符号? (y/N): ", "cyan"))).toLowerCase() ===
      "y";
    const excludeSim =
      (
        await question(color("排除易混淆字符? (y/N): ", "cyan"))
      ).toLowerCase() === "y";
